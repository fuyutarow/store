...
import datetime
import sys
#from geocording_cacco import geocoder
def expel(string,
...
csvwriter = csv.writer(codecs.open('suumo151202.csv','w',encoding="utf-8"))
...
#    try:
#        text = soup.find('th',text = i).next_sibling.next_sibling.get_text()
#        text = text.rsplit()[0]#.replace(" ",'').replace("　",'').replace("\r",'').replace("\n",'').replace("\t",'')
#    except:
#        datum = 'NA'
#    data.append(datum)
